...
VIEWS.crm = async function () {
  ...
  $('#topbar-actions').innerHTML = `${adminBtn}<button class="btn ghost" id="add-contact">＋ 担当者</button> <button class="btn" id="add-company">＋ 顧客を追加</button>`;
  $('#add-company').onclick = () => companyForm({ customer_type: CRM_TAB });
  ...
  $('#view').innerHTML = `
    <div class="panel">
      ...
      <div class="toolbar" style="margin-top:14px"><div class="search"><span class="si">🔍</span><input id="crm-search" placeholder="名前・電話・メールで検索…"></div></div>
      <div class="table-wrap"><table>
        <thead><tr><th>名前</th><th>電話</th><th>メール</th><th>進行中商談</th><th></th></tr></thead>
...
function renderCompanies() {
  ...
  const rows = CACHE.companies
    .filter(c => (c.customer_type || 'btob') === CRM_TAB)
...
window.showCompany = async (id) => {
  ...
  const prog = (c.progress || []).map(p => `
    <div class="list-item">
      <div class="li-ico ic-violet">📌</div>
      <div class="li-main"><div class="li-title">${esc(p.status) || '(進捗)'}</div><div class="li-sub">${esc(p.note) || ''}</div></div>
      <div class="li-right">${fmtDate(p.log_date)} <button class="icon-btn" onclick="delProgress(${p.id},${id})">🗑</button></div>
    </div>`).join('') || '<div class="muted" style="padding:10px">まだ進捗の記録がありません</div>';
  const typeLabel = (c.customer_type === 'btoc') ? '🧑 個人（BtoC）' : '🏢 法人（BtoB）';
  ...
  openModal(c.name, `
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:6px">
      ...
      <div><div style="font-size:17px;font-weight:700">${esc(c.name)}</div><div class="muted" style="font-size:12.5px">${typeLabel}${c.industry ? ' ・ ' + esc(c.industry) : ''}</div></div>
    </div>
    ...
    <dl class="kv">
      ...
      ${c.customer_type === 'btoc' ? '' : `<dt>Web</dt><dd>${c.website ? `<a href="${esc(c.website)}" target="_blank">${esc(c.website)}</a>` : '—'}</dd>`}
      <dt>メモ</dt><dd>${esc(c.notes) || '—'}</dd>
    </dl>
    <div class="modal-section-title">📅 進捗状況（日付ごと） <button class="btn sm soft" style="margin-left:8px" onclick="addProgress(${id})">＋ 進捗を追加</button></div>
    ${prog}
...
window.addProgress = (cid) => {
  openModal('進捗を追加', `
    <div class="grid2">
      <div class="field"><label>日付 *</label><input id="pg-date" type="date" value="${todayStr()}"></div>
      <div class="field"><label>区分 / 見出し</label><input id="pg-status" placeholder="例）初回訪問・提案・契約"></div>
    </div>
    <div class="field"><label>詳細メモ</label><textarea id="pg-note" placeholder="その日の進捗・やりとり"></textarea></div>
    <div class="form-actions"><button class="btn ghost" onclick="showCompany(${cid})">戻る</button><button class="btn" id="pg-save">追加</button></div>`);
  $('#pg-save').onclick = async () => {
    const d = $('#pg-date').value;
    if (!d) { toast('日付を入力してください', '✏️'); return; }
    await api.post(`/api/companies/${cid}/progress`, { log_date: d, status: $('#pg-status').value, note: $('#pg-note').value });
    toast('進捗を追加しました'); showCompany(cid);
  };
};
window.delProgress = async (id, cid) => { if (confirm('この進捗を削除しますか？')) { await api.del('/api/progress/' + id); toast('削除しました'); showCompany(cid); } };
...
function companyForm(data = {}) {
  const editing = !!data.id;
  const type = data.customer_type || 'btob';
  openModal(editing ? '顧客を編集' : '顧客を追加', `
    <div class="field"><label>顧客区分</label>
      <select id="c-type">
        <option value="btob" ${type === 'btob' ? 'selected' : ''}>🏢 法人（BtoB）</option>
        <option value="btoc" ${type === 'btoc' ? 'selected' : ''}>🧑 個人（BtoC）</option>
      </select></div>
    <div class="field"><label id="c-name-label">企業名 *</label><input id="c-name" value="${esc(data.name)}"></div>
    <div class="grid2">
      <div class="field" id="c-ind-wrap"><label>業種</label><input id="c-ind" value="${esc(data.industry)}"></div>
      <div class="field"><label>電話</label><input id="c-phone" value="${esc(data.phone)}"></div>
    </div>
    <div class="grid2">
      <div class="field"><label>メール</label><input id="c-email" value="${esc(data.email)}"></div>
      <div class="field"><label>住所</label><input id="c-addr" value="${esc(data.address)}"></div>
    </div>
    <div class="field" id="c-web-wrap"><label>Webサイト</label><input id="c-web" value="${esc(data.website)}"></div>
  ...
  const applyType = () => {
    ...
    $('#c-name-label').textContent = (t === 'btoc') ? 'お客様氏名 *' : '企業名 *';
    $('#c-ind-wrap').style.display = (t === 'btoc') ? 'none' : '';
    $('#c-web-wrap').style.display = (t === 'btoc') ? 'none' : '';
  };
  $('#c-type').onchange = applyType; applyType();
  $('#c-save').onclick = async () => {
    const name = $('#c-name').value.trim();
    if (!name) { toast('名前を入力してください', '✏️'); return; }
    const body = { name, customer_type: $('#c-type').value, industry: $('#c-ind').value, phone: $('#c-phone').value, email: $('#c-email').value, address: $('#c-addr').value, website: $('#c-web').value, notes: $('#c-notes').value };
